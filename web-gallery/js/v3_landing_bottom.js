...
var Registration = (function () {
    ...
    function b(n, o, m) {
        if (typeof n == "string") {
            $("#registration-form").replaceWith(n);
            ...
            if (n.registrationErrors) {
                l();
                $.each(n.registrationErrors, function (p, q) {
                    i(p, q)
                })
            }
            if (n.registrationMessages) {
                $.each(n.registrationMessages, function (p, q) {
                    d(p, q)
                })
            }
            if (n.registrationCompletionRedirectUrl) {
                window.location = n.registrationCompletionRedirectUrl
            }
        }
    }
    ...
    function i(m, n) {
        if (typeof (h[m]) != "undefined") {
            h[m].before('<div class="field-error">' + n + "</div>");
            h[m].addClass("error");
            if (m == "registration_captcha") {
                RecaptchaUtil.reloadRecaptcha();
                $("#captcha-overlay").addClass("error");
                g()
            } else {
                if (m == "registration_parent_email") {
                    f()
                }
            } if (h[m].is("input")) {
                h[m].focus()
    ...
    function d(m, n) {
        if (m == "registration.captcha_required" && n == "true") {
            g()
        }
    }
    ...
    return {
        ...
        submit: function (m) {
            if (!k) {
              ...
              //  $.post("/registration/submit", $("#register-new-user").serialize()).done(b).fail(a).always(c)
            }
            m.preventDefault();
            return false
        },
        setRecaptchaKey: function (m) {
            j = m
        },
        ...
        onWillBecomeActive: function () {
            if (!e) {
             //   RecaptchaUtil.showRecaptcha("captcha-container", j);
                e = true
            }
        },
        onBecomeActive: function () {
            $("#registration-birthday select:first-child").focus();
...
var LandingPage = (function () {
    ...
    return {
        ...
        scrollToRegistration: function (h) {
            ...
            AnimationHelper.animate($("#magnifying-glass"), {
                top: "152px"
            }, null, g[2]);
            $("#registration-form").css({
                visibility: "visible"
